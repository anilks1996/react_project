...
import { Card, CardBody, Col, Modal, ModalBody, ModalFooter, ModalHeader, Row, Spinner, Table } from 'reactstrap';
...
import { findSentWorkflowByEmployeeId } from './inboxEmployeeSlice';
import { Divider } from '@mui/material';
import DateDisplay from './DateDisplay';
import { getActiveUserId } from '../../auth/auth_slice/loginUserSlice';


const SentboxEmployeeUser = () => {
    ...
    const {sentItems,sloading} = useSelector((state)=>state.allstorereducer.inboxEmployee);
    ...
    React.useEffect(()=>{
        ...
        dispatch(findSentWorkflowByEmployeeId(activeUserId));    
    ...
    const columns = [
      ...
      {field: 'userStatus',headerName: 'Status',width: 200,
        renderCell: (params) => (
          <div>
          { (params.row && params.row.currentStatus==='Pending' || params.row.currentStatus==='Request' || params.row.currentStatus==='Requested' || params.row.currentStatus==='Forward' || 
            params.row.currentStatus==='Work Forwarded' || params.row.currentStatus==='Forward To Purchase' || params.row.currentStatus==='Forward To GEM' || params.row.currentStatus==='Forwarded')?
              <div>  
                ...
                {"  "}<button className='btn btn-warning pending-icon p-1'><HiOutlineViewfinderCircle style={{width:'20px',height:'25px'}} onClick={(e)=>{searchFun(params.row.userData)}}/></button>{" "}
  ...
  return (
    ...
    <div>
      <Card className='form-shadow p-2 mt-0' style={{minWidth:'72rem', marginLeft:'1rem',minHeight:'40rem'}}>
         {
            sloading==false ? 
            <div>
              <ParentChildDataGrid data={sentItems} onRowClick={handleRowClick} />
            </div>
            :
            <div>
...
export default SentboxEmployeeUser;
