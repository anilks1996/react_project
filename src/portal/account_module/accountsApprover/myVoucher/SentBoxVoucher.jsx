...
import { findInboxVoucherWorkflowByEmployeeId, findSentVoucherWorkflowByEmployeeId } from '../../../employeeusr_module/inboxEmployeeSlice';
import DateDisplay from '../../../employeeusr_module/DateDisplay';
import { findLeaveAppItemByLeaveAppId } from '../../../leave_module/leave_redux/slices/leave_slice/leaveMasterSlice';

const SentBoxVoucher = (props) => {
    ...
    const {inboxEmployees,sentVoucherItems,svloading} = useSelector((state)=>state.allstorereducer.inboxEmployee);
    ...
    useEffect(()=>{
      ...
      window.scrollTo({top:'0', left:'0', behavior:'smooth'});
        ...
        dispatch(findSentVoucherWorkflowByEmployeeId(activeUserId));
        //alert(props.value)
    ...
    const columns = [
        ...
        <div>
          {
            ...
            (params.row && params.row.modelType===1)?           
                <div>  
                    <b style={{color:'purple'}}> Leave Application </b> : {" "}[{params.row.modelCode}] 
                    <Divider />
                ...
                <div>
                    ...
                    <div>  
                        <b style={{color:'purple'}}> Bill Submission </b> : {" "}[{params.row.modelCode}] 
                        <Divider />
                        {params.row.itemDetails1}
                    </div>
                        : 
      ...
      {field:'actionDate',headerName: 'Time', width: 210,
        renderCell: (params)=>(            
    ...
    const ParentChildDataGrid = ({ data, onRowClick}) => {
      ...
      return (
        <div style={{ height: '100%', width: '100%' }}>
          ...
          onRowClick={(row)=>onRowClick(row)} checkboxSelection style={{backgroundColor:{getRowClassName}, fontSize:'0.86rem'}}
  ...
  return (
    ...
    <div>
      <Card className='form-shadow p-2 mt-0' style={{minWidth:'70rem', marginLeft:'1rem',minHeight:'35rem'}}>
      {/* <p><DateDisplay date={new Date()}/> </p>  */}      
         {
            svloading===false? 
            <div>
              <ParentChildDataGrid data={sentVoucherItems} onRowClick={handleRowClick} />
            </div>
            :
            <div>
              <Row className='mt-4'>
