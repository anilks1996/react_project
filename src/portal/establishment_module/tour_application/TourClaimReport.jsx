...
import { findTourByStatus, findTourByStatusOrFinancialYear, findTourClaimByStatusOrFinancialYear } from './tour_redux/tourSlices';

const TourClaimReport = () => {
    ...
    const {tourClaim,tourClaims,tourClaimUodateLoading}=useSelector((state)=>state.allstorereducer.tours);
    ...
    const getTourClaimReport=()=>{
      const tourO={...tourObj,["accCompanyFinancialYear"]:fyearValue,"code":"sbi",
        ["status"]:statusValue,
      }
      //alert(fyearValue+", "+statusValue)
      dispatch(findTourClaimByStatusOrFinancialYear({param1:statusValue, param2:fyearValue}));
    }
    ...
    <div>
      <form>
        <Card style={{minWidth:'80rem', fontSize:'0.9rem'}}>
            ...
            <CardBody>
                <Table>
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <Button className='btn btn-warning' onClick={getTourClaimReport}>Get Result</Button>
                      </td>
                    </tr>
                  </tbody>
                </Table>
            ...
            <CardBody>
            {
              tourClaims && tourClaims.length>0?
            <Table striped>
              <thead>
                <tr>
                  <th>Index</th>
              ...
              <tbody>
              {
                tourClaims && tourClaims.map((tour,index)=>(
                  <tr key={tour.id}>
                    <td>{index+1}</td>
...
export default TourClaimReport;
