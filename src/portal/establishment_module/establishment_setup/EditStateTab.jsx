...
import { deleteState, showState } from '../establishment_redux/slices/establishment_slice/countryStateCityLocationSlice';
    ...
    <div>
        <form style={{minHeight:'40rem', minWidth:'80rem'}} className='form-shadow'>
            ...
            <Card>
                ...
                <div>             
                    {
                        showModel && <CreateStateModel closeModel={closeModel}/>
                    }
                </div>
                    <Table>
                        <thead>
                        ...
                        <Table striped>
                            ...
                            <tbody>
                            {
                                states && states.map((ele,index)=>(
                                    <tr key={ele.id}>
                                        <td>{index+1}</td>
                                        <td>{ele.country && ele.country.id}</td>
                                        <td>{ele.code}</td>
                                        <td>{ele.name}</td>
                                        <td>
                                            <Link to={`/establishmentSetup/editState/${ele.id}`}><FaPencilAlt color='green'/></Link> 
