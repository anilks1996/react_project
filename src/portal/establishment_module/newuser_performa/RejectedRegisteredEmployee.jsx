...
import { showUserRegistersByRejectedStatus } from '../establishment_redux/slices/user_performa/UserRegisterSlice';


const RejectedRegisteredEmployee = () => {
    const navigatePage=useNavigate();
    const goback=()=>{
        window.history.back();
    }
    const {rejectedUserRegisters} = useSelector((state)=>state.allstorereducer.userRegister);
    ...
    useEffect(()=>{
      ...
      dispatch(showUserRegistersByRejectedStatus("Rejected"));
      dispatch(showBanks());
    ...
    const columns = [
      ...
      {field: 'userStatus',headerName: 'Actions',width: 250,
        renderCell: (params) => (
          ...
          */}
            <span className='rejected-icon'><BsFillExclamationTriangleFill />{params.row.userStatus} </span>
  ...
  return (
    <div>
      <Card className='form-shadow p-2' style={{minWidth:'70rem', marginLeft:'1rem'}}>
         {
            rejectedUserRegisters.length>0? 
            <div>
              <ParentChildDataGrid data={rejectedUserRegisters} onRowClick={handleRowClick} />
            </div>
            :
            <div>
              <p><Spinner></Spinner></p>  
...
export default RejectedRegisteredEmployee;
