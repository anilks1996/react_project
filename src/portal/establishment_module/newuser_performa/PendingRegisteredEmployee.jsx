...
import { showUserRegistersByPengingStatus } from '../establishment_redux/slices/user_performa/UserRegisterSlice';


const PendingRegisteredEmployee = () => {
    const navigatePage=useNavigate();
    const goback=()=>{
        window.history.back();
    }
    const {pendingUserRegisters} = useSelector((state)=>state.allstorereducer.userRegister);
    ...
    useEffect(()=>{
      ...
      dispatch(showUserRegistersByPengingStatus("Pending"));
      dispatch(showBanks());
  ...
  return (
    <div>
      <Card className='form-shadow p-2' style={{minWidth:'70rem', marginLeft:'1rem'}}>
         {
            pendingUserRegisters.length>0? 
            <div>
              <ParentChildDataGrid data={pendingUserRegisters} onRowClick={handleRowClick} />
            </div>
            :
            <div>
              <p><Spinner></Spinner></p>  
          ...
          {/* Modal */}
            <Modal isOpen={modal} toggle={toggle} size='xl'>
            {
              ...
              <ModalHeader className={headerBack}>User Register [ Name: {updateUserRegister.name} ] | {updateUserRegister.status}</ModalHeader>
              :
              ...
              <ModalFooter className={headerBack}>
...
export default PendingRegisteredEmployee;
